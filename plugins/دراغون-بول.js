/*
• تم التعديل بواسطة Felix
• أمر دراغون بول - يرسل مقطع عشوائي من القائمة 🎥
*/
...
let handler = async (m, { conn }) => {
  try {
    ...
    await conn.sendMessage(m.chat, {
      ...
      caption: `🔥 *دراغون بول - لقطة عشوائية!*`,
      footer: 'ᖇYᘔO ᗷOT ✨',
      gifPlayback: true
    }, { quoted: m });
    ...
    console.error(e);
    await m.react('❌');
    await conn.reply(m.chat, '⚠️ حدث خطأ أثناء إرسال الفيديو.', m);
  }
};

handler.help = ['edit7', 'دراغون-بول'];
handler.tags = ['anime'];
handler.command = /^(edit7|دراغون-بول)$/i;
